...
static enum np_scene_build_result collect_surface(
	...
	struct np_box *scene_damage)
{
	struct np_backend_scene_source source;
	if (!surface->has_published ||
	    !np_backend_describe_scene_source(surface, &source))
	...
	struct np_surface *surface, uint32_t presentation_id, int *wait_fd)
{
	...
	if (!surface || !presentation_id ||
	    !np_backend_surface_has_current(surface))
		return NP_SCENE_INVALID;
	struct np_surface *sources[] = {surface};
	enum np_backend_hold_result held = np_backend_hold_scene(
		surface, presentation_id, sources, 1);
	return held == NP_BACKEND_HOLD_READY ? NP_SCENE_READY :
	       held == NP_BACKEND_HOLD_NO_MEMORY ? NP_SCENE_NO_MEMORY :
	       NP_SCENE_INVALID;
}
	...
	struct np_scene_packet *packet, int *wait_fd)
{
	...
	if (!bytes) return NP_SCENE_NO_MEMORY;
	struct np_surface *sources[NP_SCENE_MAX_LAYERS];
	for (uint32_t i = 0; i < count; i++) sources[i] = items[i].surface;
	enum np_backend_hold_result held = np_backend_hold_scene(
		root, presentation_id, sources, count);
	if (held != NP_BACKEND_HOLD_READY) {
		free(bytes);
		return held == NP_BACKEND_HOLD_NO_MEMORY
			? NP_SCENE_NO_MEMORY : NP_SCENE_INVALID;
	}
...
{
	np_backend_release_scene(root, presentation_id);
}

void np_scene_discard_presentations(struct np_surface *surface)
{
	np_backend_discard_scenes(surface);
}
